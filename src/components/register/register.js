...
const Register = () => {
    ...
    const register = () => {
        ...
        if( name && email && password && (password === reEnterPassword)){
            axios.post("http://localhost:9002/adminRegister", user)
            .then( res => {
                alert(res.data.message)
                {res.data.message === "User already registerd as Partner" ? history.push("/partnerLogin") : history.push("/adminLogin")}
            })
        } else {
            alert("invlid input")
        }
    ...
    return (
        <div className="register">
            ...
            <div className="button" onClick={register} >Admin Register</div>
            <div>or</div>
